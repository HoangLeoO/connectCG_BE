...
public interface GroupRepository extends JpaRepository<Group, Integer> {
        ...
        @Query("SELECT g FROM Group g " +
                        ...
                        "ORDER BY gm.role ASC, g.createdAt DESC")
        org.springframework.data.domain.Page<Group> findMyGroups(@Param("userId") Integer userId,
                        org.springframework.data.domain.Pageable pageable);

        @Query("SELECT g FROM Group g " +
                        "JOIN GroupMember gm ON g.id = gm.group.id " +
                        "WHERE gm.user.id = :userId AND gm.status = 'ACCEPTED' AND (gm.role = 'ADMIN' OR g.owner.id = :userId) AND g.isDeleted = false "
                        +
                        "ORDER BY g.createdAt DESC")
        org.springframework.data.domain.Page<Group> findMyManagedGroups(@Param("userId") Integer userId,
                        org.springframework.data.domain.Pageable pageable);

        @Query("SELECT g FROM Group g " +
                        "JOIN GroupMember gm ON g.id = gm.group.id " +
                        "WHERE gm.user.id = :userId AND gm.status = 'ACCEPTED' AND gm.role = 'MEMBER' AND g.owner.id != :userId AND g.isDeleted = false "
                        +
                        "ORDER BY g.createdAt DESC")
        org.springframework.data.domain.Page<Group> findMyJoinedGroups(@Param("userId") Integer userId,
                        org.springframework.data.domain.Pageable pageable);
